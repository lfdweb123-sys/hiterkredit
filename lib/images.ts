// All images sourced from Unsplash (unsplash.com/license): free for commercial
// use, no attribution required. Served directly from Unsplash's CDN.

export const IMAGES = {
  heroFamily: {
    src: 'https://images.unsplash.com/photo-1600880292203-757bb62b4baf?q=80&w=1400&auto=format&fit=crop',
    width: 1400,
    height: 1050,
  },
  howItWorks: {
    src: 'https://images.unsplash.com/photo-1554224155-6726b3ff858f?q=80&w=1200&auto=format&fit=crop',
    width: 1200,
    height: 900,
  },
  featuresSecurity: {
    src: 'https://images.unsplash.com/photo-1563986768609-322da13575f3?q=80&w=1200&auto=format&fit=crop',
    width: 1200,
    height: 900,
  },
  aboutTeam: {
    src: 'https://images.unsplash.com/photo-1661956602868-6ae368943878?q=80&w=1400&auto=format&fit=crop',
    width: 1400,
    height: 1050,
  },
  aboutMission: {
    src: 'https://images.unsplash.com/photo-1450101499163-c8848c66ca85?q=80&w=1200&auto=format&fit=crop',
    width: 1200,
    height: 900,
  },
  aboutValues: {
    src: 'https://images.unsplash.com/photo-1600880292089-90a7e086ee0c?q=80&w=1200&auto=format&fit=crop',
    width: 1200,
    height: 900,
  },
  applyHero: {
    src: 'https://images.unsplash.com/photo-1556742044-3c52d6e88c62?q=80&w=1200&auto=format&fit=crop',
    width: 1200,
    height: 900,
  },
  contactHero: {
    src: 'https://images.unsplash.com/photo-1423666639041-f56000c27a9a?q=80&w=1200&auto=format&fit=crop',
    width: 1200,
    height: 900,
  },
  simulatorSide: {
    src: 'https://images.unsplash.com/photo-1579621970563-ebec7560ff3e?q=80&w=1200&auto=format&fit=crop',
    width: 1200,
    height: 900,
  },
  trustBackground: {
    src: 'https://images.unsplash.com/photo-1444653614773-995cb1ef9efa?q=80&w=1600&auto=format&fit=crop',
    width: 1600,
    height: 700,
  },
  
  heroHandover: {
    src: 'https://i.wpimg.pl/1200x/totalcms.wpcdn.pl/articles/a46a992ecd11839bbcf839f9f6b1155d.jpg',
    width: 1200,
    height: 1050,
  },
  } as const;

// Vidéo courte (muette, en boucle) illustrant le thème "finance", utilisée en
// arrière-plan d'une section de la page d'accueil. Hébergée sur le CDN de
// Mixkit (mixkit.co/license) : usage commercial libre, aucune attribution
// requise.
export const FINANCE_VIDEO = {
  src: 'https://assets.mixkit.co/videos/23168/23168-360.mp4',
  poster: 'https://assets.mixkit.co/videos/23168/23168-thumb-360-2.jpg',
} as const;
